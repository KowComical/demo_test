(() => {
  ...
  const STORAGE_KEY = "dice_3d_negative_review_v2_1_session";
  const LOCALE_STORAGE_KEY = "dice_3d_pilot_locale";
  const THEME_STORAGE_KEY = "dice_3d_pilot_theme";
  const REVIEWER_STORAGE_KEY = "dice_3d_pilot_reviewer_id";
  ...
  const DOUBT_OPTIONS = [
    ...
    {
      value: "negative_too_obvious",
      labels: {
        en: "Negative is implausibly obvious",
        zh: "Negative 过于离谱，很容易排除",
        ja: "Negative が極端すぎて簡単に除外できる",
        es: "El negativo es demasiado obvio",
      },
    },
    {
  ...
  const I18N = {
    en: {
      documentTitle: "DICE-3D Negative Corrections v2.1",
      languageLabel: "Language",
      themeLabel: "Theme",
      themeNeutral: "Neutral",
      themeSage: "Sage",
      themePaper: "Paper",
      pageTitle: "Which video is more internally consistent?",
      taskLine: "Review only the corrected candidates; use Not sure whenever the difference is unclear.",
      transcriptLabel: "Speech transcript",
      sideTranscript: (side) => `Video ${side} transcript`,
      videoA: "Video A",
      videoB: "Video B",
      ...
      videosReadyIndependent: "Ready. The two speech tracks differ; play A and B separately with their own controls.",
      playBoth: "Play",
      pauseBoth: "Pause",
      restartBoth: "Restart",
    ...
    zh: {
      documentTitle: "DICE-3D Negative 修正筛选 v2.1",
      languageLabel: "语言",
      themeLabel: "配色",
      themeNeutral: "清爽",
      themeSage: "浅绿",
      themePaper: "纸感",
      pageTitle: "哪个视频内部更一致？",
      taskLine: "这里只检查修正题；差异不清楚时请直接选择不确定。",
      transcriptLabel: "台词文本",
      sideTranscript: (side) => `视频 ${side} 台词`,
      videoA: "视频 A",
      videoB: "视频 B",
      ...
      videosReadyIndependent: "已就绪。两段语音不同，请分别播放 A 和 B；各自使用自己的音频与台词。",
      playBoth: "播放",
      pauseBoth: "暂停",
      restartBoth: "重新播放",
      timelineLabel: "时间轴",
    ...
    ja: {
      ...
      sideTranscript: (side) => `動画 ${side} の発話`,
      videoA: "動画 A",
      videoB: "動画 B",
      ...
      videosReadyIndependent: "準備完了。音声が異なるため、A と B をそれぞれ再生してください。",
      playBoth: "再生",
      pauseBoth: "一時停止",
      restartBoth: "最初から再生",
    ...
    es: {
      ...
      sideTranscript: (side) => `Transcripción del video ${side}`,
      videoA: "Video A",
      videoB: "Video B",
      ...
      videosReadyIndependent: "Listo. Los audios son distintos; reproduce A y B por separado.",
      playBoth: "Reproducir",
      pauseBoth: "Pausar",
  ...
  const els = {
    ...
    sideTranscriptA: $("#sideTranscriptA"),
    sideTranscriptB: $("#sideTranscriptB"),
    sideTranscriptALabel: $("#sideTranscriptALabel"),
    sideTranscriptBLabel: $("#sideTranscriptBLabel"),
    videoATranscript: $("#videoATranscript"),
    videoBTranscript: $("#videoBTranscript"),
  ...
  function refreshProgressText() {
    ...
    const reviewCondition =
      session.mode === "review" && trial
        ? `${conditionLabel(trial.negative_type)}${trial.candidate_rank ? ` · #${trial.candidate_rank}` : ""}`
        : "";
  ...
  function applyLocale() {
    ...
    els.sideTranscriptALabel.textContent = text("sideTranscript", "A");
    els.sideTranscriptBLabel.textContent = text("sideTranscript", "B");
  ...
  function makeTrial(pool, index) {
    ...
    const videoATranscript = videoA.audio_window_transcript || videoA.transcript || "";
    const videoBTranscript = videoB.audio_window_transcript || videoB.transcript || "";
    ...
    return {
      ...
      candidate_rank: pool.candidate_rank || "",
      candidate_origin: pool.candidate_origin || "",
      source_segment_id: pool.source_segment_id || "",
      source_anchor_sem_word: pool.source_anchor_sem_word || "",
      source_transcript: pool.source_transcript || "",
      ...
      video_a_transcript: videoATranscript,
      video_b_transcript: videoBTranscript,
      playback_mode: pool.playback_mode || (videoATranscript === videoBTranscript ? "synchronized" : "independent"),
    };
  }
  ...
  function updateMaxVideoTime() {
    ...
    const current = Math.max(...videos().map((video) => Number(video.currentTime || 0)));
  ...
  function showTrial() {
    ...
    const independentAudio = trial.playback_mode === "independent";
    els.transcriptCard.classList.toggle("hidden", independentAudio);
    els.sideTranscriptA.classList.toggle("hidden", !independentAudio);
    els.sideTranscriptB.classList.toggle("hidden", !independentAudio);
    els.videoATranscript.textContent = trial.video_a_transcript || "";
    els.videoBTranscript.textContent = trial.video_b_transcript || "";
    els.playbackControls.classList.toggle("hidden", independentAudio);
    ...
    els.videoA.controls = inspectMode || independentAudio;
    els.videoB.controls = inspectMode || independentAudio;
    els.videoA.muted = false;
    els.videoB.muted = !independentAudio;
  ...
  function submitTrial() {
    ...
    const response = {
      ...
      candidate_rank: trial.candidate_rank,
      candidate_origin: trial.candidate_origin,
      source_segment_id: trial.source_segment_id,
      source_anchor_sem_word: trial.source_anchor_sem_word,
      source_transcript: trial.source_transcript,
      ...
      video_a_transcript: trial.video_a_transcript,
      video_b_transcript: trial.video_b_transcript,
      playback_mode: trial.playback_mode,
  ...
  function jsonpBackendRequest(endpoint, params, timeoutMs = 5000) {
    return new Promise((resolve) => {
      const callbackName = `__dice_3d_backend_${Math.random().toString(36).slice(2)}_${Date.now()}`;
      let settled = false;
  ...
  function markVideoReady(video) {
    ...
    if (videoReady.A && videoReady.B) {
      mediaReady = true;
      const independentAudio = currentTrial()?.playback_mode === "independent";
      ...
      if (independentAudio) {
        ...
        setPlaybackControlsEnabled(false);
        setMediaStatus(text("videosReadyIndependent"), false);
      } else {
  ...
  [els.videoA, els.videoB].forEach((video) => {
    ...
    video.addEventListener("pause", () => {
      if (currentTrial()?.playback_mode === "independent") {
        flushWatchTimer();
      } else {
        updatePlaybackLabels();
      }
    ...
    video.addEventListener("play", () => {
      if (currentTrial()?.playback_mode === "independent") {
        const other = video === els.videoA ? els.videoB : els.videoA;
